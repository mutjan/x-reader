...
# 5. 提交到 GitHub
echo "[4/5] 推送到 GitHub..."
git add index.html news_data.json x_events_data.json
git commit -m "[$VERSION] Update news: $(date +%Y-%m-%d)" || echo "无变更需要提交"
git push origin main || echo "推送失败，请检查网络"

# 6. 等待 GitHub Pages 部署
echo "[5/5] 等待 GitHub Pages 部署..."
sleep 10
