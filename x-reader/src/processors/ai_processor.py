    ...
    def _load_scoring_config(self) -> Dict:
        ...
        if config is None:
            ...
            return {
                ...
                "penalty_rules": [
                    {"name": "military_political", "keywords": ["军事", "政治打压", "国际冲突"], "penalty": -15, "max_grade": "A"},
                    {"name": "metaverse_vr", "keywords": ["元宇宙", "metaverse"], "penalty": -20, "max_grade": "B"}
                ]
            }
        return config

    def _log_filtered_news(self, item, rule_name: str, matched_keywords: list):
        """记录硬过滤拦截日志到 logs/filtered_news.log，支持30天自动轮转"""
        log_file = "logs/filtered_news.log"
        os.makedirs("logs", exist_ok=True)

        # 30天轮转：如果文件存在且超过30天未轮转，备份旧文件
        if os.path.exists(log_file):
            mtime = os.path.getmtime(log_file)
            if time.time() - mtime > 30 * 24 * 3600:
                backup_name = f"{log_file}.{datetime.fromtimestamp(mtime).strftime('%Y%m%d')}"
                try:
                    os.rename(log_file, backup_name)
                    logger.info(f"过滤日志轮转: {log_file} → {backup_name}")
                except OSError as e:
    ...
    def parse_scoring_response(self, response_text: str, processed_items: List[ProcessedNewsItem]) -> List[ProcessedNewsItem]:
        ...
        try:
            ...
            for result in results:
                ...
                # 硬过滤检查：在所有评分调整之前执行，拦截非科技内容
                if "hard_filter_rules" in self.config:
                    filtered = False
                    for filter_rule in self.config["hard_filter_rules"]:
                        rule_name = filter_rule.get("name", "unknown")
                        keywords = filter_rule.get("keywords", [])
                        exclude_keywords = filter_rule.get("exclude_keywords", [])

                        # 检查是否匹配关键词
                        matched = [kw for kw in keywords if kw.lower() in content_lower]
                        if matched:
                            # 检查排除关键词（科技词汇）
                            excluded = any(ekw.lower() in content_lower for ekw in exclude_keywords)
                            if not excluded:
                                logger.info(f"硬过滤拦截 [{rule_name}]: {item.original_title[:50]}...")
                                self._log_filtered_news(item, rule_name, matched)
                                filtered = True
                                break
                    if filtered:
                        continue  # 拦截，不入库

                # 应用降分规则（penalty_rules）-- 在 special_bonuses 之前执行
                penalty_matched_rule = None
                if "penalty_rules" in self.config:
                    for rule in self.config["penalty_rules"]:
                        rule_keywords = rule.get("keywords", [])
                        penalty = rule.get("penalty", 0)
                        rule_max_grade = rule.get("max_grade", None)
                        is_penalty_matched = any(kw in content_lower for kw in rule_keywords)
                        if is_penalty_matched:
                            # 执行分数惩罚
                            if penalty != 0:
                                original_score = ai_score
                                ai_score = max(ai_score + penalty, 0)  # 最低不低于0分
                                if ai_score < original_score:
                                    rule_name = rule.get("name", "unknown")
                                    logger.info(f"降分规则 [{rule_name}]: {original_score} → {ai_score} (penalty={penalty})")
                            # 记录命中的 max_grade 规则（在等级计算后应用）
                            if rule_max_grade:
                                penalty_matched_rule = rule
                            break  # 只应用第一个匹配的规则
                ...
                # 校准调整：基于历史数据分布分析自动校准分数 (Phase 20)
                ai_score = self.calibration_engine.apply_calibration_raw(
                    ai_score, item.entities, item.original_title
                )
                ...
                # 等级上限：penalty_rules 的 max_grade 限制
                if penalty_matched_rule and penalty_matched_rule.get("max_grade"):
                    grade_order = {"S": 5, "A+": 4, "A": 3, "B": 2, "C": 1}
                    cap_grade = penalty_matched_rule["max_grade"]
                    if grade_order.get(final_grade, 0) > grade_order.get(cap_grade, 0):
                        original_grade = final_grade
                        final_grade = cap_grade
                        rule_name = penalty_matched_rule.get("name", "unknown")
                        logger.info(f"等级上限 [{rule_name}]: {original_grade} → {final_grade}")

                if final_grade == "C":
                    continue  # C级直接过滤

                # 分数上限应用：所有调整后，根据等级限制分数上限
                score_cap_config = self.config.get("score_cap_by_grade", {})
                if score_cap_config and final_grade in score_cap_config:
                    cap = score_cap_config[final_grade]
                    if cap > 0 and ai_score > cap:
                        original_score = ai_score
                        ai_score = cap
                        # cap触发后重新计算等级
                        if ai_score < thresholds.get(final_grade, 0):
                            # 分数被降低，需要重新确定等级
                            if ai_score >= thresholds.get("S", 90):
                                final_grade = "S"
                            elif ai_score >= thresholds.get("A+", 85):
                                ...
                                final_grade = "C"
                        logger.info(f"分数上限 [{final_grade}]: {original_score} → {ai_score} (cap={cap})")
